...
$adid=$_GET['adid'];
$q = mysql_query("select website,user_id from ides where adid='$adid'");
$row=mysql_fetch_array($q);
$adv=$row['website'];
$uidd=$row['user_id'];
$site=mysql_query("select website from users where user_id='$uidd'");
$sitelink=mysql_result($site,0);
...
if($value!=0 && $ipval<3)
{
if(mysql_query("INSERT INTO alldet(Country,City,Browser,Os,Date,Time,Adv,Pub,ip,site) VALUES('$info->country','$info->city','$br','$os',' $date','$date1','$adv','$pub','$ip','$sitelink')"))
 {
    $value=$value-1;
